globals [
  gini-index-reserve
  lorenz-points
  maximum-sugar-endowment ;; added here as I removed its slider from the Interface
]
...
to setup
  clear-all
  set increase increase ;; added to use it as a variable
  set maximum-sugar-endowment (minimum-sugar-endowment * (1 + (increase / 100))) ;;calculate max-sugar-endowment using the increase variable
...
to-report sugar-histogram ;; add reporter to analyze distribution over wealth deciles
  let bins 10  ;; define number of bins
  let max-sugar max [sugar] of turtles
  let bin-size (max-sugar + 1) / bins

  ;; Create a list with counts of turtles by bin
  ...
  ;; value to export in Behaviour Space
  report hist-values
end
